package com.tomorrowgold.pay.unionpay.service.impl;
...
import com.tomorrowgold.pay.common.constants.PayWay;
import com.tomorrowgold.pay.common.model.PayOrder;
import com.tomorrowgold.pay.common.util.CommonUtil;
import com.tomorrowgold.pay.unionpay.service.IUnionPayService;
import com.tomorrowgold.pay.unionpay.util.AcpService;
import com.tomorrowgold.pay.unionpay.util.DemoBase;
import com.tomorrowgold.pay.unionpay.util.SDKConfig;
import com.tomorrowgold.pay.unionpay.util.UnionConfig;
@Service
public class UnionPayServiceImpl implements IUnionPayService{
     ...
     */
	@Override
	public String unionPay(PayOrder product) {
		...
		//前台回调地址(自定义)
		String frontUrl = "http://www.meilaobossxiaohai.xyz/springboot_pay/";
		...
		requestData.put("txnAmt", CommonUtil.subZeroAndDot(product.getTotalAmount()));             //交易金额，单位分，不要带小数点
	...
	public String query(PayOrder product) {
		...
		*/
		...
		/***商户接入参数***/
		data.put("merId", UnionConfig.merId);                  //商户号码，请改成自己申请的商户号或者open上注册得来的777商户号测试
		data.put("accessType", "0");                           //接入类型，商户接入固定填0，不需修改
		
		/***要调通交易以下字段必须修改***/
		data.put("orderId", product.getOutTradeNo());                 //****商户订单号，每次发交易测试需修改为被查询的交易的订单号
		data.put("txnTime", product.getTxnTime());                 //****订单发送时间，每次发交易测试需修改为被查询的交易的订单发送时间
