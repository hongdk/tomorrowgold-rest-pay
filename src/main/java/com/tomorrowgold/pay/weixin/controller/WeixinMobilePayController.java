package com.tomorrowgold.pay.weixin.controller;
...
import com.tomorrowgold.pay.common.util.AddressUtils;
import com.tomorrowgold.pay.common.util.DateUtil;
import com.tomorrowgold.pay.weixin.service.IWeixinPayService;
import com.tomorrowgold.pay.weixin.util.ClientCustomSSL;
import com.tomorrowgold.pay.weixin.util.ConfigUtil;
import com.tomorrowgold.pay.weixin.util.HttpUtil;
import com.tomorrowgold.pay.weixin.util.PayCommonUtil;
import com.tomorrowgold.pay.weixin.util.XMLUtil;
import com.tomorrowgold.pay.weixin.util.mobile.MobileUtil;
...
public class WeixinMobilePayController {
    ...
    public String  pay(PayOrder product,ModelMap map) {
		logger.info("H5支付(需要公众号内支付)");
		String url =  weixinPayService.pubAccountPay(product);
		return "redirect:"+url;
    }
    ...
    public String  h5pay(PayOrder product,ModelMap map) {
		logger.info("纯H5支付(不建议在APP端使用)");
		//mweb_url为拉起微信支付收银台的中间页面，可通过访问该url来拉起微信客户端，完成支付,mweb_url的有效期为5分钟。
		String mweb_url =  weixinPayService.h5Pay(product);
		if(StringUtils.isNotBlank(mweb_url)){
			return "redirect:"+mweb_url;
		}else{
			return "redirect:"+"https://自定义错误页面";//自定义错误页面
		}
    }
    ...
    public String  smallRoutine(PayOrder product,ModelMap map) {
